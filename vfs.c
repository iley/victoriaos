...
#define set_mode(new_mode) {if(mode != MODE_NONE) { printf("Error: You must specify only ONE mode option\n"); return 1; } mode = (new_mode); }
...
Options: -h show this help\n\
	...
	if(!img_filename || (!filename && (mode == MODE_READ || mode == MODE_WRITE || mode == MODE_DELETE))) {
		printf(help_message);
		return 1;
	}
